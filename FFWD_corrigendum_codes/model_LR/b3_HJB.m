...
for itHJB = 1:maxitHJB
    ...
    dVdB_f(:,:,1:nval_B-1,:) = (V0(:,:,2:nval_B,:)-V0(:,:,1:nval_B-1,:))/dB;                          % zero in the upper boundary
    dVdN_f(:,:,:,1:nval_N-1) = (V0(:,:,:,2:nval_N)-V0(:,:,:,1:nval_N-1))/dN;                          % zero in the upper boundary
    % backward differences
    dVda_b(2:nval_a,:,:,:)   = (V0(2:nval_a,:,:,:)-V0(1:nval_a-1,:,:,:))/da;
    dVda_b(1,:,:,:)          = (w(1,:,:,:).*z(1,:,:,:) + r(1,:,:,:).*amin).^(-gamma);                 % state constraint boundary condition
    dVdB_b(:,:,2:nval_B,:)   = (V0(:,:,2:nval_B,:)-V0(:,:,1:nval_B-1,:))/dB;                          % zero in the lower boundary
    dVdN_b(:,:,:,2:nval_N)   = (V0(:,:,:,2:nval_N)-V0(:,:,:,1:nval_N-1))/dN;                          % zero in the lower boundary
    ...
    elem_m_f     = elem_m .* (elem_m>0); % forward
    elem_m_b     = elem_m .* (elem_m<0); % backward
...
% build the slightly different version of A1 that will be used in the KFE
