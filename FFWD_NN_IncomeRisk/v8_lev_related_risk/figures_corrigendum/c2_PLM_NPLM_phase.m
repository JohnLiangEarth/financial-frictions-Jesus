...
phase_diagram_ready = ~isempty(SSS_points);
if ~phase_diagram_ready
    fprintf(2,['WARNING: No intersection between the PLM = 0 and mu^N = 0 ' ...
        'curves was detected on the fine grid. Skipping steady-state-dependent figures.\n']);
    warning('PhaseDiagram:NoStochasticSteadyState', ...
        ['No intersection between the PLM = 0 and mu^N = 0 curves was ' ...
         'detected on the fine grid. Skipping steady-state-dependent figures.']);
else
...
myfig=figure(91);
set(myfig, 'Position', [0 0 900 900])
...
xlabel('debt ($B$)', 'interpreter','latex','FontSize',14);
ylabel('equity ($N$)', 'interpreter','latex','FontSize',14);
title('(d) upwind, $nval_B=16$', 'interpreter','latex','FontSize',14)
xlim([Bmin Bmax])
ylim([Nmin Nmax])
...
text(1.35,2.45,'SSS', 'interpreter','latex','HorizontalAlignment','left','FontSize',12)

legend({'$h(B,N)=0$','$\mu^N(B,N)=0$','Stochastic steady state','Deterministic steady state'},'Location','northeast', 'interpreter','latex','FontSize',11)
grid

print -dpdf g91_phase
savefig(myfig,'g91_phase.fig');
